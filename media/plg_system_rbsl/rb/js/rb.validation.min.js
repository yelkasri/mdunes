...
if($this.attr("required")!==undefined||$this.attr("aria-required")!==undefined||$this.hasClass('required')){message=settings.builtInValidators.required.message;if($this.data("validationRequiredMessage")){message=$this.data("validationRequiredMessage");}
$this.data("validationRequiredMessage",message);}
if(($this.attr("type")!==undefined&&$this.attr("type").toLowerCase()==="number")||$this.hasClass('validate-number')){message=settings.builtInValidators.number.message;if($this.data("validationNumberMessage")){message=$this.data("validationNumberMessage");}
$this.data("validationNumberMessage",message);}
if(($this.attr("type")!==undefined&&$this.attr("type").toLowerCase()==="email")||$this.hasClass('validate-email')){message="Not a valid email address<!-- data-validator-validemail-message to override -->";if($this.data("validationValidemailMessage")){message=$this.data("validationValidemailMessage");}else if($this.data("validationEmailMessage")){message=$this.data("validationEmailMessage");}
...
(settings.options.prependExistingHelpBlock?$helpBlock.data("original-contents"):""));}}else{$controlGroup.removeClass("warning error success");if(value!=null&&value.length>0){$controlGroup.addClass("success");}
...
$.fn.jqBootstrapValidation=function(method){if(defaults.methods[method]){return defaults.methods[method].apply(this,Array.prototype.slice.call(arguments,1));}else if(typeof method==='object'||!method){return defaults.methods.init.apply(this,arguments);}else{$.error('Method '+method+' does not exist on jQuery.jqBootstrapValidation');return null;}};$.jqBootstrapValidation=function(options){$(":input").not("[type=image],[type=submit]").jqBootstrapValidation.apply(this,arguments);};$(document).ready(function(){$('.rb-validate-form').find("input,textarea,select").not('.no-validate').jqBootstrapValidation();});})(rb.jQuery);var rb_credit_card=function()
{var rb_default_card_format=/(\d{1,4})/g;var cards=[{type:'visaelectron',pattern:/^4(026|17500|405|508|844|91[37])/,format:rb_default_card_format,length:[16],cvcLength:[3],luhn:true},{type:'maestro',pattern:/^(5(018|0[23]|[68])|6(39|7))/,format:rb_default_card_format,length:[12,13,14,15,16,17,18,19],cvcLength:[3],luhn:true},{type:'forbrugsforeningen',pattern:/^600/,format:rb_default_card_format,length:[16],cvcLength:[3],luhn:true},{type:'dankort',pattern:/^5019/,format:rb_default_card_format,length:[16],cvcLength:[3],luhn:true},{type:'visa',pattern:/^4/,format:rb_default_card_format,length:[13,16],cvcLength:[3],luhn:true},{type:'mastercard',pattern:/^5[0-5]/,format:rb_default_card_format,length:[16],cvcLength:[3],luhn:true},{type:'amex',pattern:/^3[47]/,format:/(\d{1,4})(\d{1,6})?(\d{1,5})?/,length:[15],cvcLength:[3,4],luhn:true},{type:'dinersclub',pattern:/^3[0689]/,format:rb_default_card_format,length:[14],cvcLength:[3],luhn:true},{type:'discover',pattern:/^6([045]|22)/,format:rb_default_card_format,length:[16],cvcLength:[3],luhn:true},{type:'unionpay',pattern:/^(62|88)/,format:rb_default_card_format,length:[16,17,18,19],cvcLength:[3],luhn:false},{type:'jcb',pattern:/^35/,format:rb_default_card_format,length:[16],cvcLength:[3],luhn:true}];var getCard=function(number)
{var card,_i,_len;number=(number+'').replace(/\D/g,'');for(_i=0,_len=cards.length;_i<_len;_i++){card=cards[_i];if(card.pattern.test(number)){return card;}}
return null;},getType=function(number)
{var _ref;if(!number){return null;}
return(_ref=getCard(number))!=null?_ref.type:null;},isValidLuhn=function(num)
{var digit,digits,odd,sum,_i,_len;odd=true;sum=0;digits=(num+'').split('').reverse();for(_i=0,_len=digits.length;_i<_len;_i++){digit=digits[_i];digit=parseInt(digit,10);if((odd=!odd)){digit*=2;}
if(digit>9){digit-=9;}
sum+=digit;}
return sum%10===0;};return{getCard:getCard,getType:getType,isValidLuhn:isValidLuhn,};}
var Rb_FormValidator=function(){var $,handlers,inputEmail,custom,setHandler=function(name,fn,en){en=(en==='')?true:en;handlers[name]={enabled:en,exec:fn};},findLabel=function(id,form){var $label,$form=jQuery(form);if(!id){return false;}
$label=$form.find('#'+id+'-lbl');if($label.length){return $label;}
$label=$form.find('label[for="'+id+'"]');if($label.length){return $label;}
return false;},handleResponse=function(state,$el,type,msg){if($el.attr('data-rb-validate-error')){var $error=$($el.attr('data-rb-validate-error'));var $label=false;}else if($el.attr('id')){var $label=$el.data('label');var $error=$('[for="'+$el.attr('id')+'"]').not($label);}else{var $error=$('[for="'+$el.selector+'"]');var $label=false;}
type=typeof type!=='undefined'?type:'error';msg=typeof msg!=='undefined'?msg:$el.attr('error-message');if(state===false){$el.addClass('invalid').attr('aria-invalid','true');if($error){$error.removeClass('hide');$error.addClass('show').html(msg);}
if($label){$label.addClass('invalid').attr('aria-invalid','true');}}else{$el.removeClass('invalid').attr('aria-invalid','false');if($error){$error.removeClass('show');$error.addClass('hide')}
if($label){$label.removeClass('invalid').attr('aria-invalid','false');}}},validate=function(el){var $el=jQuery(el),tagName,handler;if($el.attr('disabled')){handleResponse(true,$el);return true;}
...
if((handler)&&(handler!=='none')&&(handlers[handler])&&$el.val()){if(handlers[handler].exec($el,$el.val())!==true){handleResponse(false,$el);return false;}}
handleResponse(true,$el);return true;},isValid=function(form){var valid=true,i,message,errors,error,label;jQuery.each(jQuery(form).find('input, textarea, select, fieldset, button'),function(index,el){if(($(el).is(':visible')||$(el).hasClass('validate-hidden'))&&validate(el)===false){valid=false;}});jQuery.each(custom,function(key,validator){if(validator.exec()!==true){valid=false;}});if(!valid){scrollToError(form);}
return valid;},scrollToError=function(form){errors=jQuery(form).find("input.invalid, textarea.invalid, select.invalid, fieldset.invalid, button.invalid");var el=$(errors[0]);if($(el).hasClass('validate-hidden')){var elOffset=$('[for="'+el.attr('id')+'"]').offset().top;}else{var elOffset=el.offset().top;}
var elHeight=el.height();var windowHeight=$(window).height();var offset;if(elHeight<windowHeight){offset=elOffset-((windowHeight/2)-(elHeight/2));}
else{offset=elOffset;}
$('html, body').animate({scrollTop:offset},1000);},attachToForm=function(form){var inputFields=[];$(form).find('input, textarea, select, fieldset, button').each(function(){var $el=$(this),id=$el.attr('id'),tagName=$el.prop("tagName").toLowerCase();if(!$el.is(':visible')&&!$el.hasClass('validate-hidden')){return true;}
if($el.hasClass('required')){$el.attr('aria-required','true').attr('required','required');}
if((tagName==='input'||tagName==='button')&&$el.attr('type')==='submit'){if($el.hasClass('validate')){$el.on('click',function(){return isValid(form);});}}else{if($el.hasClass('validate-hidden')){$el.on('change',function(){return validate(this);});}
...
$el.data('label',findLabel(id,form));inputFields.push($el);}});$(form).data('inputfields',inputFields);},initialize=function(selector){$=jQuery.noConflict();handlers={};custom=custom||{};inputEmail=(function(){var input=document.createElement("input");input.setAttribute("type","email");return input.type!=="text";})();setHandler('username',function(element,value){regex=new RegExp("[\<|\>|\"|\'|\%|\;|\(|\)|\&]","i");return!regex.test(value);});setHandler('password',function(element,value){regex=/^\S[\S ]{2,98}\S$/;return regex.test(value);});setHandler('numeric',function(element,value){regex=/^(\d|-)?(\d|,)*\.?\d*$/;return regex.test(value);});setHandler('integer',function(element,value){regex=/^(\d|-)?(\d|,)*\d*$/;return regex.test(value);});setHandler('email',function(element,value){regex=/^[a-zA-Z0-9.!#$%&’*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/;return regex.test(value);});setHandler('image',function(element,value){var imageSize=0;var fileField=element[0];for(i=0;i<fileField.files.length;i++){imageSize+=fileField.files[i].size;}
return(element.data('fileuploadlimit')>imageSize);});setHandler('min',function(element,value){regex=/^(\d|-)?(\d|,)*\.?\d*$/;return(regex.test(value)&&value>element.data('rb-min'));});setHandler('rb-credit-card',function(element,value){value=(value+'').replace(/\D/g,'');var rb_card=new rb_credit_card();var card=rb_card.getCard(value);if(!card){return false;}
if(-1==rb.jQuery.inArray(value.length,card.length)){return false;}
if(!rb_card.isValidLuhn(value)){return false;}
return true;});setHandler('rb-cvc-length',function(element,value){var card_number,card_selector=element.data('rb-validate'),rb_card=new rb_credit_card();card_number=rb.jQuery(card_selector).val();if(!card_number){return true;}
var card=rb_card.getCard(card_number);if(!card){return true;}
if(-1==rb.jQuery.inArray(value.length,card.cvcLength)){return false;}
return true;});setHandler('rb-regex-pattern',function(element,value){var regex_pattern,regex_pattern_string=element.data('validate-pattern');if(!regex_pattern_string){return true;}
regex_pattern=new RegExp(regex_pattern_string);return regex_pattern.test(value);});setHandler('rb-exp-date',function(element,value){var month_number,year_number,date,mm,yyyy,month_el,year_el;var l=element.data('rb-validate-type');if('month'===element.data('rb-validate-type')){month_el=element;year_el=rb.jQuery(element.data('rb-validate'));}else{year_el=element;month_el=rb.jQuery(element.data('rb-validate'));}
month_number=month_el.val();year_number=year_el.val();if(!month_number&&!year_number){return false}
date=new Date();mm=date.getMonth()+1;yyyy=date.getFullYear();year_number=parseInt(year_number);month_number=parseInt(month_number);if(year_number<yyyy){return false;}
if(year_number==yyyy&&month_number<=mm){return false;}
handleResponse(true,month_el);handleResponse(true,year_el);return true;});jQuery(selector).each(function(){attachToForm(this);},this);};return{initialize:initialize,isValid:isValid,validate:validate,setHandler:setHandler,attachToForm:attachToForm,custom:custom,handleResponse:handleResponse,scrollToError:scrollToError};};