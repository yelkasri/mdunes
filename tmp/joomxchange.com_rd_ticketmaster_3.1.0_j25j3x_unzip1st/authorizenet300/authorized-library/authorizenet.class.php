...
class authorizenet_class {
   ...
   function gatewaytype($type) {
	  ...
	  // The sandbox type will be received here, you can choose if you want a 
	  // sandbox test or a real transaction. By changing gateway you MUST change
	  // the login information also. Otherwise it won't work!!
	
	  if ($type == 1){
		  ## Sandbox has been turned on, so use the test account. ;)
		  $this->gateway_url = "https://test.authorize.net/gateway/transact.dll";
	  }else{
		  ## A real transaction should be made now :)
