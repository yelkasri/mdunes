 ...
 */
 function plgRDMediaRDMpaypal( &$subject, $params  ) {
	...
	$this->ipn_on 			= $this->params->def( 'ipn_on', 0 );	
	$this->mail_on_ipn 		= $this->params->def( 'mail_on_ipn', 0 );	
	...
	## IPN messenger URL:
	$this->notify_url = JURI::root().'index.php?option=com_ticketmaster&controller=ipn&task=ipnProcessor&plg=rdmpaypal';
		...
		switch($result){
			...
			case "VERIFIED":
				...
				if($results->total < 1){
					...
					$payment_details = http_build_query($data);
					$payment_type 	 = 'PayPal-IPN';
					$orderid 		 = $data['custom'];
									
					## Now store all data in the transactions table
					$row->transid 	= $data['txn_id'];
					$row->details 	= $request;
					$row->amount 	= $data['mc_gross'];
									
					## DO NOT CHANGE THE TYPE !! OTHERWISE YOUR PAYPAL PLUGIN IS NOT DOING THE CORRECT CHECK!!
					## IN THE PLUGIN SCRIPT WE ARE QUERYING ON "PayPal-IPN" IF THE IPN IS FASTER THEN THE RETURN WE CAN STILL GIVE THE CORRECT STATUS OF THE PAYMENT@!
					$row->type = 'PayPal-IPN';
								
					$row->email_paypal = $data['payer_email'];
					$row->orderid	   = $data['custom'];
					...
					$query = 'UPDATE #__ticketmaster_orders'
						  ...
						  . ' WHERE ordercode = '.(int)$data['custom'].'';
					...
					$query = 'SELECT * FROM #__ticketmaster_orders WHERE ordercode = '.(int)$data['custom'].'';
					...
					$creator = new sendonpayment( (int)$data['custom'] );
					$creator->send();
					...
					if($this->mail_on_ipn == 1) { 
						
						## Get the email address from the configuration:
						$mainframe = JFactory::getApplication();
						
						
						$sql = "SELECT * FROM #__ticketmaster_emails WHERE emailid = ".(int)$this->notify_email_msg."";
						$db->setQuery($sql);
						$config = $db->loadObject();

						if($this->notify_email_msg != 0) {

							$message = str_replace('%%TID%%', $data['txn_id'], $config->mailbody);
							$message = str_replace('%%OID%%', $data['custom'], $message);
							$message = str_replace('%%AMOUNT%%', $data['mc_gross'], $message);
							$message = str_replace('%%DATE%%', $data['payment_date'], $message);
							$message = str_replace('%%NAME%%', $data['first_name'].' '.$data['last_name'], $message);
							$message = str_replace('%%EMAIL%%', $data['payer_email'], $message);						
							
							## Imaport mail functions:
							jimport( 'joomla.mail.mail' );
		
							## Set the sender of the email:
							$sender[0]  = $mainframe->getCfg('mailfrom');;
							$sender[1]  = $mainframe->getCfg('fromname');;
							## Compile mailer function:
							$obj = JFactory::getMailer();
							$obj->setSender( $sender );
							$obj->isHTML( true );
							$obj->setBody ( $message );
							$obj->addRecipient($this->notify_email);
							## Add reply to and subject:
							$obj->addReplyTo($this->notify_email);
							$obj->setSubject($this->notify_subject.': '.$data['custom']);
		...
		if($status == 200 AND strpos($response, 'SUCCESS') === 0) {
			...
			if ($response['payment_status'] != 'Completed') {
				...
				if($results->total > 0){				
					...
					## Check that txn_id has not been previously processed
					$sql = 'SELECT * FROM #__ticketmaster_transactions AS t WHERE t.transid = "'.$payment_id.'" AND t.type = "PayPal-IPN" ';
					...
					if( count($result) > 0){
						...
						## THE IPN SCRIPT WAS FASTER THEN THE RETURN --> SHOW ONLY THE THANK YOU MESSAGE
