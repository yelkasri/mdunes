...
class confirmation{					
	...
	public function doConfirm() {
		...
		$pdf->Write(0, '10001' );			
			...
			$pdf->SetXY(27, $height2);
			$pdf->Write(0, utf8_decode(JText::_( 'COM_TICKETMASTER_COUPONCODE'). ': '. $disco->coupon_code ));
		...
		$total 		= _getAmount($this->eid, 0, 1);
		$fees 		= _getFees($this->eid, 0, 1);
		$discount 	= _getDiscount($this->eid, 0, 1);		
	...
	public function doSend() {
		...
		## Get the paid amount:
		$to_be_paid = _getAmount($this->eid, 0, 1);	
		$discount 	= _getDiscount($this->eid, 1);
		$paid = $to_be_paid-$discount;
		
		$price = showprice($configuration->priceformat ,$paid , $configuration->valuta);
