...
class ticketcreator{					
	...
	function doPDF() {
		...
		$sql='SELECT  a.*, t.*, e.eventname, c.address, c.name, c.city, 
		...
		if (strpos($order->name_position,'-') !== false) {
			...
			$pdf->Write(0, utf8_decode($order->name).' - '.utf8_decode($order->address).' - '.utf8_decode($order->city));		
		}
		...
		if ($order->seat_sector != 0) {
		    ...
		    $seat = $db->loadObject();				  			
		 	if (strpos($order->position_seatnumber,'-') !== false) {
				...
				$pdf->Write(0, JText::_( 'COM_TICKETMASTER_PDF_SEAT_NUMBER' ).' '. $seat->seatid);	
			}
		}
		...
		if (strpos($order->location_position,'-') !== false) {
			...
			$pdf->SetFont('Arial', 'B', $order->ticket_fontsize+1);
			$pdf->SetTextColor($order->ticket_fontcolor_r, $order->ticket_fontcolor_g, $order->ticket_fontcolor_b);
