...
class special{					
	...
	public function create() {
		...
		$pdf->SetTextColor(0,0,0);
		$pdf->SetXY(10, 75);
		$pdf->Write(0, JText::_( 'COM_TICKETMASTER_ENTRANCE_TICKETS_FOR').' '.$this->eid);		
		## Writing the name customer:
		$pdf->SetXY(10, 80);
		$pdf->Write(0, utf8_decode(JText::_( 'COM_TICKETMASTER_TICKETS_FOR_CUSTOMER')).' '.utf8_decode($order->name));
		$pdf->SetXY(10, 85);
		$pdf->Write(0, JText::_( 'COM_TICKETMASTER_TICKETS_ORDER_DATE').' '.$orderdate);
		$pdf->SetXY(10, 90);
		
		## Writng the total order price.
		if ($config->use_euros_in_pdf == 2) {
			## Fixing the euro issue..
			$price = showprice($config->priceformat ,$total , '');
			$pdf->Write(0, JText::_( 'COM_TICKETMASTER_TICKETS_ORDERPRICE').': '.chr(128).' '.$price);
		}elseif ($config->use_euros_in_pdf == 3){
			$price = showprice($config->priceformat ,$total , '');
			$pdf->Write(0, JText::_( 'COM_TICKETMASTER_TICKETS_ORDERPRICE').': '.chr(0x00A3).' '.$price);
		}else{
			$pdf->Write(0, JText::_( 'COM_TICKETMASTER_TICKETS_ORDERPRICE').': '.$price);
		}	
		...
		for ($i = 0, $n = count($data); $i < $n; $i++ ){
			...
			if ($row->seat_sector != 0) {
				...
				if ($row->parentname != $row->ticketname) {
					$pdf->SetFont('Arial', '', 9);
					$pdf->Write(0, utf8_decode($row->eventname).' ('.utf8_decode($row->eventcode).') - '. utf8_decode($row->ticketname).' - '.$seatnumber);
				}else{
					$pdf->SetFont('Arial', '', 9);
					$pdf->Write(0, utf8_decode($row->eventcode).' / '.utf8_decode($row->ticketname).' - '.$seatnumber);
				}				
			...
			if ($config->use_euros_in_pdf == 2) {
				...
				$pdf->Write(0, JText::_( 'COM_TICKETMASTER_ORDER' ).' '.$row->orderid.' :: '.JText::_( 'COM_TICKETMASTER_PDF_PRICE' ).' '.chr(128).' '.$price.' :: '.JText::_( 'COM_TICKETMASTER_PDF_DATE' ).' '.$ticketdate);
			}elseif ($config->use_euros_in_pdf == 3){
				$price = showprice($config->priceformat ,$order->ticketprice , '');
				$pdf->Write(0, JText::_( 'COM_TICKETMASTER_ORDER' ).' '.$row->orderid.' :: '.JText::_( 'COM_TICKETMASTER_PDF_PRICE' ).' '.chr(0x00A3).' '.$price.' :: '.JText::_( 'COM_TICKETMASTER_PDF_DATE' ).' '.$ticketdate);
			}else{
				$pdf->Write(0, JText::_( 'COM_TICKETMASTER_ORDER' ).$row->orderid.' :: '.JText::_( 'COM_TICKETMASTER_PDF_PRICE' ).' '.$price.' :: '.JText::_( 'COM_TICKETMASTER_PDF_DATE' ).' '.$ticketdate);
			}	
			...
			$pdf->SetFont('Arial', '', 9);
			$pdf->Write(0, JText::_( 'COM_TICKETMASTER_BARCODE' ).': '. $row->barcode);
