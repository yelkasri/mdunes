...
class ticketmasterModelTicketbox extends JmodelLegacy
{
   ...
   function getList() {
		...
		if (empty($this->_data)) {
			...
			## Making the query for showing all the clients in list function
			$sql='SELECT a.*, t.ticketname, e.eventname, c.address, c.name, c.city, SUM(t.ticketprice) AS orderprice, 
   ...
   function getData() {
		...
		if (empty($this->_data)) {
			...
			## Making the query for showing all the clients in list function
			$sql='SELECT a.*, t.ticketname, t.ticketprice, e.eventname, c.address, c.name, c.city
			      FROM #__ticketmaster_orders AS a, #__ticketmaster_clients AS c,
				  #__ticketmaster_events AS e, #__ticketmaster_tickets AS t'
				  .$where; 
   ...
   function getExtData() {
		...
		if (empty($this->_data)) {
			...
			$sql='SELECT c.id, c.orderid, c.seatid
	...
	function paymentsdone($cid = array(), $paid = 1) {
		...
		if (count( $cid )) {
			...
			if ($config->payment_email_send == 1){
				...
				$query = 'SELECT o.*, c.emailaddress, c.name 
